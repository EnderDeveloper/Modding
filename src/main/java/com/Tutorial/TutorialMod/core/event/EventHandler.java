package com.Tutorial.TutorialMod.core.event;
    ...
    private static void setBlocks(IWorld world, BlockPos pos, Random rand) {
        for (int x = -5; x < 5; x++) {
            for (int y = -5; y < 5; y++) {
                for (int z = -5; z < 5; z++) {
                    world.setBlockState(
                            pos.add(x, y, z), BlockInit.SPIN_BLOCK.get().getDefaultState()
                                    .with(SpinBlock.HORIZONTAL_FACING, Direction.Plane.HORIZONTAL.random(rand)),
